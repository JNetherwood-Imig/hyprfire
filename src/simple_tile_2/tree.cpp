 ...
 */
class tile_view_animation_t : public wf::hyprfire::TransformAnimation
{
  public:
    using wf::hyprfire::TransformAnimation::TransformAnimation;
    ...
    ~tile_view_animation_t()
    {
        ...
        // start.
        view->get_transformed_node()->rem_transformer<hyprfire::TransformNode>();
...
bool view_node_t::needs_crossfade()
{
    ...
    if (view->has_data<wf::hyprfire::TransformAnimation>())
    {
        return true;
    }
...
static nonstd::observer_ptr<wf::hyprfire::TransformAnimation> ensure_animation(
    wayfire_toplevel_view view, wf::option_sptr_t<wf::animation_description_t> duration)
{
    if (!view->has_data<wf::hyprfire::TransformAnimation>())
    {
        view->store_data<wf::hyprfire::TransformAnimation>(
            std::make_unique<tile_view_animation_t>(view, duration));
    }

    return view->get_data<wf::hyprfire::TransformAnimation>();
}
